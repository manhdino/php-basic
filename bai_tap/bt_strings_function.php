 ...
 * lưu ý khi xóa chữ thì phải xóa cả dấu cách cạnh nó
 * Input: Tạ Hoàng An
 * Output: Hoàng An
 * */
 ...
 //Xử lý:
 //Tìm vị trí dấu cách đầu tiên và sau đó tách ra từ vị trí đó
 $positionSpace = mb_strpos($strInput, ' ', 0,'UTF-8');
 $firstWord = mb_substr($strInput, 0, $positionSpace+1, 'UTF-8');
 //positionSpace + 1 vì phải xóa cả dấu cách cạnh nó
...
//Xử lý chuỗi
//1. Lấy chữ đầu tiên
$positionSpaceFirst = mb_strpos($strInput, ' ',0, 'UTF-8');
$firstWord = mb_substr($strInput, 0, $positionSpaceFirst, 'UTF-8');

//2. Lấy chữ cuối cùng
$positionSpaceEnd = mb_strripos($strInput, ' ', 0, 'UTF-8');
...
//Handle
$positionSpaceLast = mb_strripos($fullName, ' ', 0, 'UTF-8');
...
for ($i = 0; $i<mb_strlen($password, 'UTF-8'); $i++){
    ...
    if (mb_strpos($symbol, $char, 0, 'UTF-8')!=false){
        $checkSymbol = true;
    }
}
